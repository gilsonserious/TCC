...
export class PedidosComponent implements OnInit {
  ...
  aceitar(id) {
    this.afs.firestore.collection('pedidos').doc(id).update({ status: 'Agendado' })
      .then(() => {
        this.carregar()
      })
  }
  ...
  recusar(id) {
    this.afs.firestore.collection('pedidos').doc(id).update({ status: 'Recusado (Agendamento encerrado para horário)' })
      .then(() => {
        this.carregar()
      })
  }
  ...
  saiuEntrega(id) {
    this.afs.firestore.collection('pedidos').doc(id).update({ status: 'Aguardando entrega' })
      .then(() => {
        this.carregar()
      })
  }
